$(document).ready(function () {
      ...
      success: function (result) {
        ...
        if (result['result'] === 'r1') {
          ...
          for (var x = 0; x < result['total']; x++) {
            ...
            action += "<a href='"+base_url+"category_child/?id_category=" + result['id'][x] + "' id='btn_detail" + result['id'][x] + "' class='fa fa-folder-open'></a> &nbsp;";
  ...
  set_active = function () {
    ...
    $.each(id, function (x, val) {
      ...
      $(document).on('click', '#btn_active' + val, function (event) {
        ...
        $.ajax({
          url: base_url + 'category/set_active',
          type: 'POST',
          data:{
            id: val
          },
  ...
  add_data = function (name, active) {
    $.ajax({
      url: base_url + 'category/add_data',
      type: 'POST',
      data: {
        name: name,
        active: active
  ...
  edit_data = function (id, name, active) {
    $.ajax({
      url: base_url + 'category/edit_data',
      type: 'POST',
      data: {
        id: id,
        name: name,
  ...
  remove_data = function () {
    ...
    $.ajax({
      url: base_url + 'category/remove_data',
      type: 'POST',
      data: {
        id: id
      },
