...
class Category_detail extends CI_Controller {
  ...
  function __construct() {
    ...
    $this->load->model('Model_category_detail');
  }
  
  public function index($id_category_child = NULL) {
    if($id_category_child == NULL){
      redirect('/category/');die();
    }
    $page = 'Category';
    $sidebar['page'] = $page;
    $content['js'] = array();
    array_push($content['js'], 'category_detail/function.js');
    array_push($content['js'], 'category_detail/init.js');
    array_push($content['js'], 'category_detail/action.js');
    $content['id_category_child'] = $id_category_child;
    
    //get category child name
    $param['id'] = $id_category_child;
    $result_data = $this->Model_category_child->get_data($param);
    if($result_data->num_rows() > 0){
      $content['category_child_name'] = $result_data->row()->name;
    }else{
      redirect('/category/');die();
    }
    //end get category child name
    
    $data['header'] = $this->load->view('header', '', TRUE);
    $data['sidebar'] = $this->load->view('sidebar', $sidebar, TRUE);
    $data['content'] = $this->load->view('category_detail/index', $content, TRUE);
    $this->load->view('template_index', $data);
  }
  
  public function get_data(){
    //param
    $param['id_category_child'] = ($this->input->post('id_category_child', TRUE)) ? $this->input->post('id_category_child', TRUE) : 0;
    ...
    //paging
    $get_data = $this->Model_category_detail->get_data($param);
    ...
    if ($get_data->num_rows() > 0) {
      $get_data_paging = $this->Model_category_detail->get_data($param, $limit, $size);
      $temp = 0;
  ...
  public function get_specific_data(){
    ...
    $result_data = $this->Model_category_detail->get_data($param);
    if($result_data->num_rows() > 0){
      $data['result'] = "r1";
  ...
  public function validate_post($param){
    ...
    if($id_products == 0){
      $data['result'] = "r2";
      $data['result_message'] .= "<strong>Products</strong> must be choosen !<br/>";
    }
  ...
  public function add_data(){
    ...
    if($validate_post['result'] == "r1"){
      $this->Model_category_detail->add_data($param);
    }
  ...
  public function edit_data(){
    ...
    if($param['id'] != ""){
      $validate_post = $this->validate_post($param);
      if($validate_post['result'] == "r1"){
        $this->Model_category_detail->edit_data($param);
      }
    }else{
  ...
  public function remove_data(){
    ...
    if($param['id'] != ""){
      $data['result'] = "r1";
      $this->Model_category_detail->remove_data($param);
    }else{
      $data['result'] = "r2";
