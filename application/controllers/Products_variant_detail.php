...
class Products_variant_detail extends CI_Controller {
  ...
  public function index() {
    ...
    $sidebar['page'] = $page;
    $content['js'] = array();
    array_push($content['js'], 'products_variant_detail/function.js');
    array_push($content['js'], 'products_variant_detail/init.js');
    array_push($content['js'], 'products_variant_detail/action.js');
    ...
    $data['content'] = $this->load->view('products_variant_detail/index', $content, TRUE);
  ...
  public function get_data() {
    ...
    if ($get_data->num_rows() > 0) {
      $get_data_paging = $this->Model_products_variant_detail->get_data($param, $limit, $size);
      $temp = 0;
  ...
  public function get_specific_data() {
    ...
    $result_data = $this->Model_products_variant_detail->get_data($param);
    if ($result_data->num_rows() > 0) {
      $data['result'] = "r1";
  ...
  public function validate_post($param, $state, $edit_size = TRUE) {
    ...
    if ($state == "add") {
      ...
      if ($sku == NULL) {
        ...
        if($check_duplicate_sku->num_rows() > 0){
          $data['result'] = "r2";
          $data['result_message'] .= "<strong>SKU</strong> is already existed !<br/>";
        }
      }
    }
  ...
  public function add_data() {
    ...
    $param['id_products'] = ($this->input->post('id_products', TRUE)) ? $this->input->post('id_products', TRUE) : 0;
    $param['id_color'] = ($this->input->post('id_color', TRUE)) ? $this->input->post('id_color', TRUE) : 0;
    $param['sku'] = ($this->input->post('sku', TRUE)) ? $this->input->post('sku', TRUE) : NULL;
  ...
  public function edit_data() {
    ...
    $check_edited_size = $this->Model_products_variant_detail->get_data($param_check);
    ...
    if ($param['id'] != "") {
      ...
      if ($validate_post['result'] == "r1") {
        $this->Model_products_variant_detail->edit_data($param);
      }
    } else {
  ...
  public function remove_data() {
    ...
    if ($param['id'] != "") {
      $data['result'] = "r1";
      $this->Model_products_variant_detail->remove_data($param);
    } else {
      $data['result'] = "r2";
      $data['result_message'] = "<strong>Data ID</strong> is not found, please refresh your browser!<br/>";
    }
